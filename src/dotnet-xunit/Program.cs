...
class Program
{
    static int Main(string[] args)
    {
        ...
        var testProject = Directory.EnumerateFiles(Directory.GetCurrentDirectory(), "*.*proj")
                                   .Where(f => !f.EndsWith(".xproj"))
                                   .SingleOrDefault();

        var projExtTarget = Path.Combine(Path.GetDirectoryName(testProject),
                                         "obj",
...
");
        ...
        {
            FileName = "dotnet",
            Arguments = $"msbuild \"{testProject}\" /t:_GetXunitTestInfo /nologo \"/p:_XunitInfoFile={tmpFile}\" /v:q"
        };
        ...
        try
        {
            ...
            lines = File.ReadAllLines(tmpFile);
        }
        finally
        {
            File.Delete(tmpFile);
        ...
        var consolePath = Path.Combine(consoleFolder, "xunit.console.dll");
